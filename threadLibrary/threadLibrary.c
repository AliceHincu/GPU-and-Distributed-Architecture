...
thread_t* thread_self() {
    return current_thread; // Returnează thread-ul curent
}
...
void thread_exit() {
    ...
    // Marchează thread-ul curent ca terminat
    current_thread->state = THREAD_FINISHED;

    // Eliberează memoria pentru stack
    free(current_thread->stack);

    // Declanșează următorul thread
    schedule_next();
}
...
void thread_function1() {
    int id = thread_self()->id;
    for (int i = 0; i < 5; i++) {
        printf("Thread %d is running (iteration %d)\n", id, i + 1);
        printf("Thread %d sleeping...", id);
        sleep_seconds(2);
        printf("Thread %d finished sleeping...", id);
    }
}
